...
ROOT_VALIDATOR_MNEMONIC=""
NUMBER_OF_VALIDATORS="1"
NUMBER_OF_ORCHESTRATORS="1"
...
PARAM_UNBONDING_TIME="10s"
PARAM_MAX_DEPOSIT_PERIOD="21600s"
PARAM_VOTING_PERIOD="10s"
...
# create zero account
(echo $KEYRING_OS_PASS; echo $KEYRING_OS_PASS) | cudos-noded keys add zero-account --keyring-backend test |& tee "${CUDOS_HOME}/zero-account.wallet"
chmod 600 "${CUDOS_HOME}/zero-account.wallet"
ZERO_ACCOUNT_ADDRESS=$(echo $KEYRING_OS_PASS | cudos-noded keys show zero-account -a --keyring-backend test)
...
for i in $(seq 1 $NUMBER_OF_VALIDATORS); do
    if [ "$i" = "1" ] && [ "$ROOT_VALIDATOR_MNEMONIC" != "" ]; then
        (echo $ROOT_VALIDATOR_MNEMONIC; echo $KEYRING_OS_PASS) | cudos-noded keys add "validator-$i" --recover --keyring-backend test
    else
        (echo $KEYRING_OS_PASS; echo $KEYRING_OS_PASS) | cudos-noded keys add "validator-$i" --keyring-backend test |& tee "${CUDOS_HOME}/validator-$i.wallet"
        chmod 600 "${CUDOS_HOME}/validator-$i.wallet"
    fi
    validatorAddress=$(echo $KEYRING_OS_PASS | cudos-noded keys show validator-$i -a --keyring-backend test)
...
for i in $(seq 1 $NUMBER_OF_ORCHESTRATORS); do
    (echo $KEYRING_OS_PASS; echo $KEYRING_OS_PASS) | cudos-noded keys add "orch-$i" --keyring-backend test |& tee "${CUDOS_HOME}/orch-$i.wallet"
    chmod 600 "${CUDOS_HOME}/orch-$i.wallet"
    orchAddress=$(echo $KEYRING_OS_PASS | cudos-noded keys show orch-$i -a --keyring-backend test)    
...
if [ "$FAUCET_BALANCE" != "" ] && [ "$FAUCET_BALANCE" != "0" ]; then
    ((echo $KEYRING_OS_PASS; echo $KEYRING_OS_PASS) | cudos-noded keys add faucet --keyring-backend test) |& tee "${CUDOS_HOME}/faucet.wallet"
    chmod 600 "${CUDOS_HOME}/faucet.wallet"
    FAUCET_ADDRESS=$(echo $KEYRING_OS_PASS | cudos-noded keys show faucet -a --keyring-backend test)
...
(echo $KEYRING_OS_PASS; echo $KEYRING_OS_PASS) | cudos-noded gentx validator-1 "${VALIDATOR_BALANCE}${BOND_DENOM}" ${ORCH_ETH_ADDRESS} ${ORCH_01_ADDRESS} --min-self-delegation 2000000000000000000000000 --chain-id $CHAIN_ID --keyring-backend test
